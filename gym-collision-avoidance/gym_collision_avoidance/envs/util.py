...
def my_maxpooling(x, out_shape, kernel_len):
    kernel=[kernel_len, kernel_len]; stride=kernel_len
    in_height, in_width = x.shape

    out_height = int((in_height - kernel[0]) / stride) + 1
    out_width  = int((in_width  - kernel[1]) / stride) + 1
    ...
    for i in range(out_height):
        for j in range(out_width):
            start_i = i * stride
            start_j = j * stride
            end_i = start_i + kernel[0]
            end_j = start_j + kernel[1]
