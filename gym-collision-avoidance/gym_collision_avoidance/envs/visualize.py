...
def plot_episode(agents, in_evaluate_mode,
    ...
    img = plt.imread(Config.MAP_FILE[0])
    ax.imshow(img, extent=[-Config.MAP_XW/2, Config.MAP_XW/2, -Config.MAP_YW/2, Config.MAP_YW/2])
    ...
    for i, agent in enumerate(agents):
        ...
        if circles_along_traj:
            plt.plot(agent.global_state_history[:agent.step_num+last_index+1, 1],
                     ...
                     color=plt_color, ls='-', linewidth=2, alpha=0.5)
            plt.plot(agent.global_state_history[0, 3],
                     agent.global_state_history[0, 4],
                     color=plt_color, marker='*', markersize=20)
            # plt.plot(agent.global_state_history[:agent.step_num+last_index+1, -2],
            #          agent.global_state_history[:agent.step_num+last_index+1, -1],
            #          color=plt_color, marker='*', markersize=5)
            ...
            if True:
                ...
                arrow_end = agent.global_state_history[ind, 1:3] + np.array([np.cos(agent.heading_global_frame), np.sin(agent.heading_global_frame)])           
                ax.add_patch(ptch.FancyArrowPatch(arrow_start, arrow_end, arrowstyle="Simple,head_width=10,head_length=20", color=c))
            ##################
