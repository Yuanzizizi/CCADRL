...
class CollisionAvoidanceEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.static_map_filename = Config.MAP_FILE
        self.map = None
        ...
        return next_observations, rewards, game_over, \
            {
                ...
                'log_info': [agent.info for agent in self.agents] 
            },  [agent.length for agent in self.agents]

    def reset(self, test_case_index=0):
        ...
        if self.episode_step_number is not None and self.episode_step_number > 0 and self.plot_episodes and self.test_case_index >= 0:
            plot_episode(self.agents, self.evaluate, self.map, self.test_case_index, self.id, 
                         circles_along_traj=Config.PLOT_CIRCLES_ALONG_TRAJ, plot_save_dir=self.plot_save_dir, plot_policy_name=self.plot_policy_name, 
                         limits=self.plt_limits, fig_size=self.plt_fig_size, show=Config.SHOW_EPISODE_PLOTS, save=Config.SAVE_EPISODE_PLOTS)
        ...
        self.update_map_in_testcase_set()
    ...
    def _take_action(self, actions, dt):
        ...
        for agent_index, agent in enumerate(self.agents):
            dis_center.append(agent.pred_t([calculate_distance(agent.pos_global_frame, [0,0]), agent.policy.nn.predict_v(self.observation[agent_index])]))
                

            if agent.goal_global_frame[1] < 0:
                agent_towards.append(1)
            else:
                agent_towards.append(0)

            agent_same_dir.append(1 if agent.goal_global_frame[1]*agent.pos_global_frame[1]>0 else 0)

            temp_distA.append(100 if agent_same_dir[-1] else dis_center[-1])
        ...
        for agent_index, agent in enumerate(self.agents):
            if agent.is_done:
                ...
                adjusted_distA = [
                    val if val == 100 or agent_towards[i] == agent_towards[agent_index] else -val
                    for i, val in enumerate(temp_distA)
                ]

                sorted_top5 = sorted(adjusted_distA, key=lambda x: abs(x))[:5]

                all_actions[agent_index, :] = agent.policy.find_next_action(dict_obs, self.agents, agent_index, self.map, np.array(sorted_top5), 
                                                                            not agent_same_dir[agent_index] and dis_center[agent_index] > dis_center_min and dis_center[agent_index] < dis_center_max)

        for i, agent in enumerate(self.agents):
            agent.take_action(all_actions[i,:], dt)

    def _update_top_down_map(self, new_pos_unavail=None):
        """ After agents have moved, call this to update the map with their new occupancies. """
        self.map.add_agents_to_map(new_pos_unavail)
        ...
        for i, agent in enumerate(self.agents):
            if agent.is_at_goal:
                if agent.was_at_goal_already is False:
                    ...
                    agent.info = ReachGoal()
                    self._fmm_update(agent.pos_global_frame)
            else:
                # agents at their goal shouldn't be penalized if someone else bumps into them
                if agent.was_in_collision_already is False:
                    if collision_with_agent[i]:
                        ...
                        agent.goal_global_frame = agent.pos_global_frame
                        agent.was_at_goal_already = True
                        
                        agent.info = Collision()
                        self._fmm_update(agent.pos_global_frame)
                        ...
                        if dist_btwn_nearest_agent[i] <= Config.GETTING_CLOSE_RANGE:
                            ...
                            agent.info = Danger(dist_btwn_nearest_agent[i])
                        ...
                        rewards[i] += (agent.last_fmm_dist_to_goal - agent.fmm_dist_to_goal) * Config.REWARD_TOWARDS_GOAL

        rewards = np.clip(rewards, self.min_possible_reward, self.max_possible_reward)
        if Config.TRAIN_SINGLE_AGENT: rewards = rewards[0]
        return rewards

    def _fmm_update(self, new_pos_unavail):
        if Config.UPDATE_FMM_DIST:
            self._update_top_down_map(new_pos_unavail)
            for a in self.agents:
                if a.is_at_goal or a.in_collision or a.ran_out_of_time:
                    continue
                a.update_fmm_dist(self.map)        
    ...
    def _check_for_collisions(self):
        ...
        for i in agent_inds:
            agent = self.agents[i]
            collision_with_wall[i] =self.map.check_collisions(agent.pos_global_frame, agent.radius)
    ...
    def update_map_in_testcase_set(self):
        self.test_case_args['static_map'] = self.map
