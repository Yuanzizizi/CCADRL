...
class OtherAgentsStatesSensor(Sensor):
    ...
    def sense(self, agents, agent_index, top_down_map=None):
        ...
        other_agents_states = np.zeros((Config.MAX_NUM_OTHER_AGENTS_OBSERVED, Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH))
        other_agent_count = 0
        for other_agent in clipped_sorted_agents:
            ...
            v_orthog_ego_frame = np.dot(other_agent.vel_global_frame,
                                        host_agent.ref_orth)
            if Config.GS_goal_known:
                rel_subgoal_to_other_global_frame = other_agent.sub_goal - host_agent.pos_global_frame
                subg_parallel_ego_frame = np.dot(rel_subgoal_to_other_global_frame, host_agent.ref_prll)
                subg_orthog_ego_frame   = np.dot(rel_subgoal_to_other_global_frame, host_agent.ref_orth)
            ...
            if Config.GS_goal_known:
                other_obs = np.array([p_parallel_ego_frame,
                                        ...
                                        v_orthog_ego_frame,
                                        subg_parallel_ego_frame - p_parallel_ego_frame,
                                        subg_orthog_ego_frame   - p_orthog_ego_frame,                                    
                                        other_agent.radius,
                                        combined_radius,
                                        dist_2_other,
                                        min(other_agent.fmm_dist_to_goal, 2)]) 
            else:
                other_obs = np.array([  p_parallel_ego_frame,
                                        p_orthog_ego_frame,
                                        v_parallel_ego_frame,
